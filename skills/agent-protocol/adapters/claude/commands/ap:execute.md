---
name: ap:execute
description: "Claim and execute existing pending tasks. Any agent."
argument-hint: "[task-id|next|--all] [--origin review|plan|import]"
---

# /ap:execute

认领并执行已有 pending task。省略参数时默认为 `next`。`--all` 是当前会话内的安全串行批处理：每轮重新读取 `tasks.json`，一次只认领一个 task，优先恢复匹配的 `in_progress` task，不并行，不启动外部 supervisor。`--origin review|plan|import` 只作为 `origin_command` 来源过滤，不表示 task 类型。本命令不创建 task，不接收直接粘贴的 execution prompt、prompt artifact、plan artifact 或 plan 文档，不支持 `--one` 或 `--loop`。
...
4. 选择执行目标：只接受 task id、`next`、`--all` 和可选 `--origin review|plan|import`。如果用户传入直接 prompt、直接 plan、prompt artifact 或 plan artifact，停止并要求先使用 `/ap:import`。
...
11. 不修改任务契约字段；执行阶段不能创建 task 或归一化外部 prompt / plan。

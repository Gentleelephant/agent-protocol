...
6. 为每个可执行问题单独生成一个 `execution_prompt` artifact，要求来源摘要清晰、边界清晰、问题描述清晰、建议修复方式清晰，并写明推荐执行命令（通常是 `/ap:execute <task-id>`；如果要批量自动修复，则由主 agent 使用 `/ap:run --origin review --all`）。

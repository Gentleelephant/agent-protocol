...
初始化项目级个人协议状态。配置命令，任何 agent 均可执行。此命令不安装项目级 `/ap:` 子命令。

用户在调用本命令时传入的文本不作为安装参数处理。
...
4. Git 仓库下更新 `.git/info/exclude` 忽略上述个人本地文件和 `.agent-memory/`。
5. 不修改团队共享的 `AGENTS.md`、`CLAUDE.md` 或其他项目公共配置。
